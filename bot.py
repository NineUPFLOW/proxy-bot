...
# Константы теперь настраиваются через переменные окружения (с дефолтами)
PUBLISH_COUNT = int(os.environ.get("PUBLISH_COUNT", 5))
CONCURRENCY = int(os.environ.get("CONCURRENCY", 10))
MAX_SOCKS5_RATIO = float(os.environ.get("MAX_SOCKS5_RATIO", 0.4))
MAX_WEB_COUNT = int(os.environ.get("MAX_WEB_COUNT", 2))  # не более N WEB-прокси за раз (нестабильны)
...
async def check_with_semaphore(sem, raw):
    async with sem:
        try:
            ...
            logger.warning(f"check error for {raw.get('ip')}:{raw.get('port')}: {e}")
            return None


async def main():
    # async-with гарантирует закрытие сессии бота даже при исключении
    # (раньше bot.session.close() вызывался вручную в нескольких местах
    # и мог не сработать при незапланированном исключении).
    async with Bot(
        token=BOT_TOKEN,
        ...
        for p in final:
            try:
                ...
                logger.error(
                    f"Ошибка публикации {p.get('protocol')} "
                    f"{p.get('ip')}:{p.get('port')}: {e}"
                )
